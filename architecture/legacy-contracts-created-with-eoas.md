...
3. The Router deploys the clone via `LegacyDeployer` with a deterministic salt (CREATE2 — the address matches the prediction shown in the UI), registers the Permit2 batch, binds the owner to the new legacy in the vault, and records consent against the published terms hash.
